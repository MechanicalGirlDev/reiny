...
use crate::launch::{LaunchPlan, OnExit, ResolvedGrain};
...
/// 1 grain を子プロセスとして起動する。
fn spawn_one(
    bin_dir: &Path,
    spec: &ResolvedGrain,
    default_log_level: Option<&str>,
) -> anyhow::Result<Child> {
    let path = resolve_bin(bin_dir, &spec.bin);
    if !path.exists() {
        anyhow::bail!(
            "binary '{}' not found at {} — build the workspace first (cargo build)",
            spec.bin,
            path.display()
        );
    }
...
fn run_inner(
    ...
    for &i in &order {
        ...
        let child = spawn_one(bin_dir, spec, default_log_level)?;
    ...
    loop {
        ...
        match spec.on_exit {
            ...
            OnExit::Respawn => {
                ...
                match spawn_one(bin_dir, spec, default_log_level) {
                    Ok(child) => {
...
pub fn run_launch(
    ...
    let result = run_inner(plan, &bin_dir, default_log_level, &stop, &mut children);
