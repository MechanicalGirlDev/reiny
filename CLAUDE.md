...
The `reiny` binary lives in **`reiny-cli`** (it depends on `reiny-launch` as a library; `reiny-launch` is now lib-only). Subcommands `new`/`init`/`add`/`check`/`build`/`run`/`compress`/`bag` plus two non-subcommand entry paths handled *before* clap in `main`:
