...
```rust
let pings = node.publish::<Ping>()?;    // Ping は [publications] → reiny/ping へ送る
let mut pongs = node.subscribe::<Pong>()?; // Pong は依存先 pong の公開型 → reiny/pong を購読
```
...
│   └── src/
│       └── main.rs     # #[reiny::main] で reiny/ping ノードを起動
└── pong/
    └── (ping と対称)
```
