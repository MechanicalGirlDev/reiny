...
| [ping-pong-workspace](ping-pong-workspace) | proto と Reiny.toml を **ワークスペースに 1 つ** 集約。型は共有カタログ `[internals]`。各 launch が proto を自前でビルド。 |
| [ping-pong-schema](ping-pong-schema) | workspace 共有 + `[schema]`。`[internals]` を **共有スキーマクレートが 1 度だけ** prost ビルドし、各 launch はそれを Cargo 依存として共有(proto 再コンパイルなし)。 |
